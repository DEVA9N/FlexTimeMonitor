...
results are shown as table. Some entries can be altered (Start, End, Discrepancy,
Note).

The collected history data is stored in an xml file in:
\My Documents\Flex Time Monitor\History.xml
...
1. Unzip files
2. Run setup.exe
3. Copy the FlexTimeMonitor link to autostart
4. done
...
on that day. The end is always the last shutdown on that same day.
...
A right click on the Flex Time Monitor icon that can be found in the systray
reveals information about the current workday. For example how many hours you
have left before you can go home.
